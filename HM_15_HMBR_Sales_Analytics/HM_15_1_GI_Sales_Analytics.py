"""
📊 H_15_1_GI_Sales_Analytics.py – Monthly Sales Analytics for GI Corporation

🚀 PURPOSE:
    - Replicate legacy GI sales report with original logic
...
📁 OUTPUT:
    - H_15_1_GI_Sales_Information.xlsx
    - H_15_1_GI_Sales_Ratios.xlsx
    - Email with HTML summary (SL No., vertical layout)

📬 EMAIL:
    - Recipients: get_email_recipients("H_15_1_GI_Sales_Analytics")
...
# === 12. Export to Excel ===
INFO_FILE = "H_15_1_GI_Sales_Information.xlsx"
RATIOS_FILE = "H_15_1_GI_Sales_Ratios.xlsx"
...
with pd.ExcelWriter(RATIOS_FILE, engine='openpyxl') as writer:
    ...
    rectproductqty = df_rectreturn.groupby(['xitem', 'xdesc'])[['xqtyord']].sum()
        ...
        # Only write non-empty DataFrames to avoid pandas MultiIndex Excel bug
...
subject = f"H_15.1 GI Sales Report – {analysis_month:02d}-{thisYear}"
body_text = "Please find the monthly sales reports for GI Corporation attached."
...
send_mail(
    ...
    html_body=[(dict_df, "Monthly GI Performance Summary")]
)

print("✅ H_15.1 completed successfully.")