"""
📊 HM_15_HMBR_Sales_Analytics.py – Monthly Sales Performance Report for HMBR

🚀 PURPOSE:
    - Use original logic from legacy script
    - Keep all calculations intact
    - Modernize: single Excel, better email, dynamic recipients
"""
...
print(f"🏢 Processing for ZID={ZID} (GULSHAN TRADING)")
...
# === 7. Fetch Data (Your Original Logic, Cleaned) ===
...
def create_opdor(zid):
    return pd.read_sql("SELECT xordernum, xdate, xcus, xdiv, xsp, xtotamt FROM opdor WHERE zid = %(zid)s", engine, params={'zid': zid})

def create_opddt(zid):
    return pd.read_sql("SELECT zid, xordernum, xitem, xqty, xrate, xdisc, xdiscf, xlineamt, xdtwotax, xdtdisc, xdtcomm FROM opddt WHERE zid = %(zid)s", engine, params={'zid': zid})
...
df_main_sale = pd.merge(df_opddt, df_opdor, on='xordernum', how='left')
...
df_main['xfinalrate'] = df_main['xfinallineamt'] / df_main['xfinalqtydel'].replace(0, 1)  # Avoid div/0
...
# === 11. Convert Summary to DataFrame (Vertical, Original Style) ===
print("📊 Preparing HTML summary...")

# Recreate dict_df exactly like original
...
# Reorder columns: SL, All_Info, then 0,1,2,3
cols = ['SL', 'All_Info'] + [col for col in dict_df.columns if col not in ['SL', 'All_Info']]
dict_df = dict_df[cols]

print(f"✅ HTML summary ready: {len(dict_df)} rows")


# === 12. Export to Single Excel ===
INFO_FILE = "HM_15_HMBR_Sales_Information.xlsx"
RATIOS_FILE = "HM_15_HMBR_Sales_Ratios.xlsx"
...
subject = f"HM_15 HMBR Sales Report – {analysis_month:02d}-{thisYear}"
body_text = "Please find the monthly HMBR sales reports attached."
...
send_mail(
    ...
    html_body=[(dict_df, "Monthly HMBR Performance Summary")]
)

print("✅ HM_15 completed successfully.")