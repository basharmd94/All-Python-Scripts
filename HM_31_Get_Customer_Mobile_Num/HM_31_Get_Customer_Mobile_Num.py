...
📅 Purpose:
Daily script to sample, deduplicate, track & email new customer orders from HMBR, GI & Zepto business units.
...
2. Sample 5 customers from each region per business (HMBR + GI + Zepto)
3. Combine → deduplicate by customer_code across all businesses
...
# Fetch & sample orders for all businesses
df_hmbr = get_sampled_orders(ZID_HMBR)
df_gi = get_sampled_orders(ZID_GI)
df_zepto = get_sampled_orders(ZID_ZEPTO)

# Combine and deduplicate by customer_code
df_combined = pd.concat([df_hmbr, df_gi, df_zepto], ignore_index=True)
df_order_details = (
    df_combined
...
if not df_new.empty:
    ...
    try:
        # Build separate HTML sections per business
        business_names = {ZID_HMBR: "HMBR", ZID_GI: "GI", ZID_ZEPTO: "ZEPTO"}
        html_sections = []
        for zid_value, biz_name in business_names.items():
            df_section = df_new[df_new['zid'] == zid_value]
            if not df_section.empty:
                html_sections.append((df_section, f"{biz_name} — Today's customers list with phone number"))
        ...
        send_mail(
            ...
            html_body = html_sections if len(html_sections) > 0 else [(df_new, f"Today's customers list with phone number")]
        )
